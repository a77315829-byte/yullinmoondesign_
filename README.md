# yullin-moon-design AI 견적 업무 보조 시스템
...
`start.bat`을 실행하면 Python GUI가 사내 LAN의 공유 서버를 자동으로 검색합니다.

- 서버가 1개면 자동으로 연결합니다.
- 서버가 여러 개면 PC 이름과 IP 목록에서 접속할 서버를 선택합니다.
- 서버가 없으면 `설정`에서 `이 PC를 공유 서버로 실행`하거나 서버 IP를 직접 입력할 수 있습니다.
- 서버장 PC의 업무 웹/API를 모든 게스트가 함께 사용하므로 메일, 분석 결과, 품목 수정과 하트 상태가 공유됩니다.
- 공유 서버는 TCP `54837`, 자동 검색은 UDP `54837`을 사용합니다.
- 분홍색 하트 상태는 서버장 PC의 `backend/data/heart_sync.db`에 저장되고 약 5초 간격으로 동기화됩니다.

서버장 PC가 꺼지면 게스트의 업무 화면 연결도 함께 중단됩니다. 서버장 설정의 `방화벽 규칙 추가`는 Windows 관리자 권한으로 TCP/UDP 54837만 허용하고, 사설 네트워크·로컬 서브넷으로 범위를 제한합니다. 공유기 포트포워딩은 사용하지 마세요.
